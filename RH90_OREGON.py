# -*- coding: utf-8 -*-
"""
Created on Fri Jul  3 12:30:18 2020
...
moose_mount_LW = np.rot90(pa.DataFrame(moose_mount_df, columns = ['LW_average']))
...
falls_creek_LW = np.rot90(pa.DataFrame(falls_creek_df, columns = ['LW_average']))
...
soapgrass_mount_LW = np.rot90(pa.DataFrame(soapgrass_mount_df, columns = ['LW_average']))
...
woods_creek_LW = np.rot90(pa.DataFrame(woods_creek_df, columns = ['LW_average']))
...
## Final variable in model_data is the RH threshold ##
## Threshold values (%) ##
## Moose Mountain = 79.25
## Woods Creek = 74.0
## Falls Creek = 77.0
## Soapgrass Mountain = 78.5
model_data, Mod_DUR = RH90(RHERA,datetime_time,77)
...
for i in range(366):
    ...
    for j in range(24):
        ...
        model_ind = np.where(((model_time >= start_seg) & (model_time < end_seg)) == True)
        validation_ind = np.where(((validation_time >= start_seg) & (validation_time < end_seg)) == True)
...
plt.title('SRC model daily LWD for given site')
plt.xlim(-0.5,25)
plt.ylim(-0.5,25)