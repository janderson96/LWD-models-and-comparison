# -*- coding: utf-8 -*-
"""
Created on Sat Jul  4 10:50:12 2020
...
# Site recorded data from oregon_trial_data.csv
...
# Convert numpy time into datetime
...
U_xs     = soap_data['u10'].values
U_ys     = soap_data['v10'].values
T_ds     = soap_data['d2m'].values - 273.15
T_s      = soap_data['t2m'].values - 273.15
ssr_s    = soap_data['ssr'].values
precip_s = soap_data['tp'].values

## Final variable in model_data is the threshold ##
## Threshold values (mm) ##
## Moose Mountain = 0.070
## Woods Crek = 0.00
## Falls Creek = 0.074
## Soapgrass Mountain = 0.00
model_data, LE_r = RES(datetime_time,U_xs,U_ys,ssr_s,T_s,T_ds,precip_s,0.0)
...
print('Average daily LWD error =', err_av)
...
plt.title('RES model daily LWD for given site')
plt.xlim(-0.5,25)
plt.ylim(-0.5,25)
